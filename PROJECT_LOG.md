...
**Дополнение (с.70):** travel/end UI — PNG-панели; travel = таймлайн эпох + «вы тут»; подписи **только в PNG** (не canvas поверх кнопок). Comic first/rebuilt — **отдельная преамбула** до меню, не блок внутри travel-меню.
...
- **Сейчас:** **Фаза 5** (с.69 таймер ✅; с.70 PNG end/travel ✅). **Дальше:** hazardLevel → шары; comic-преамбула travel.
