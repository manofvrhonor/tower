...
### ADR-15: Красные шары — коллизии и угроза (Этап 6, в работе)

**Решение (сессии 18–19):**
...
**Не делать:** включать BALL × DOME; постоянный homing к центру.
...
**Удар по шару:** `red-ball._deflectOffBat` — отскок сохраняет скорость до удара
(направление пост-столкновительное). Требует доработки (бэклог: скорость не поправилась).
...
| 7 | Предмет для отбивания (бита) | захват ✅, отбивание — доработка |
| 8 | Полировка | план |
...
- **Этап 7 (сессии 19–20):** бита-сковородка; захват **починен** (state `grabbed`,
  коллайдер на блине и ручке). Отбивание шара — есть, но скорость требует доработки.
...
- **Бита (Этап 7, ADR-16):** захват работает (десктоп), Quest QA не пройден.
- **Бэклог (заведён в сессии 20)** — см. `CURRENT_TASK.md`:
  - скорость шара при отбивании битой не поправилась (фикс не сработал);
  - меню входа + выбор уровней сложности;
  - рассинхрон видимой комнаты и физической (отскоки не по стенам);
