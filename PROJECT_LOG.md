...
**Дополнение (с.70–71):** travel/end UI — PNG-панели; travel = таймлайн + «вы тут» + home/gear/confirm; подписи **только в PNG**. Пульт открывает меню **в любой момент** после Start. PNG-плоскости **не** пересоздавать на каждый open/close (Quest Link crash, с.71). Comic first/rebuilt — **отдельная преамбула** до меню. VR-размер меню ×2 (с.71).

**Причина:** VR-читаемый цикл «собрал → киношный прыжок → другая эпоха»; запястье готовит апгрейд-слоты.
...
- Canvas/текст-оверлей поверх PNG-кнопок travel/end (двойной текст).
- Comic-кадры внутри travel-меню (→ преамбула до меню).
- Пересоздавать travel PNG-плоскости / `texture.dispose()` на каждый open/close (с.71 — Quest Link).

**Снято (с.80 ADR-26):** запрет «боковые апгрейд-слоты» — заменён принципом **parent+digit** (C1–C3).
...
### ADR-26: Сложности = маршруты L1–L5 (A→F + C1–C3) (с.80)

**Решение:**
- Ось снепа **A→B→C→D→E→F**; F = folder `attach` как A.
- Ветки **parent+digit** (`assemblyChain.branchSlots`): C1/C2/C3 — stub-кубы; слот виден/снепается только после parent C; каскад снимает детей с parent.
- 5 сложностей → 5 `routeId` (`CONFIG.assemblyRoutes` L1–L5): **spawn** деталей по эпохам ≠ **quotaStages** (travel). C*/F не в квоте.
- Якоря: A@present, F@past, E@future, C1/C2/C3 по одной на эпоху. B/C/D разъезжаются по маршруту.
- Давление сверху: ballCount, junk/decoy, `durationSec`, hardcore `ringInnerSpinMult`.
- Победа = все слоты сессии (9).

**Причина:** раньше 5 уровней отличались только шарами; нужна разная логическая задача и перенос/возвраты.

**Не делать:**
- Класть spawn-список в `locations.stageIds` (тупик на L3/L4/L5).
- C1–C3/F в travel-квоту v1.
- Ролл альтернативных пулов / decoy под C* — replay v2.

**План:** `difficulty_redesign_abcdef_*.plan.md` (UI Cursor; копия в репо по запросу).
...
- **Сейчас:** с.80 ✅ ADR-26 (A–F, C1–C3, L1–L5, tip без коллизий). **Дальше:** Quest QA маршрутов.
